...
Назад</a>
  ...
  <div class="row center-block">
    ...
    <template v-else>
    <div class="col-xs-10">
Подтвердите данные за месяц с {{response.month.firstWorkDay | moment}} по  {{response.month.lastWorkDay | moment}}
    </div>
...
<table class="table">
<tr v-for="targetValue in response.monthReportTargetValues" >
  <td><span style="color:red;" v-if="targetValue.target.enabledToEmployer==0"><i class="fa fa-exclamation-circle" aria-hidden="true"></i>
   </span> {{targetValue.target.name}}</td>
  <td style="padding-right:5px;">
    <input style="margin-bottom:5px;margin-right:5px;" class="form-control" placeholder="Наименование организации" type="text" v-model:value="targetValue.value">
  </td>
...
export default {
  ...
  methods: {
    ...
    callMonthReport: function () {
      var varthis = this
      var itemId = this.$route.params.id
      this.$parent.callAPI('GET', this.$parent.state.getMonthReportUrl + '/' + itemId).then(function (response) {
    ...
    saveReport: function (user) {
      var varthis = this
      this.$parent.callAPI('PUT', this.$parent.state.getMonthReportUrl + '/' + this.response.id, this.response).then(function (res) {
        swal(
          'Готово!',
          'Отчет успешно подтвержден!',
          'success'
        )
        varthis.$router.go(-1)
        if (res.status !== 200) {
